...
function envoyerPushTest2Veille(tokenDestinataire, to, from, callId, offerStr, notIdVal) {
    ...
    // Payload réveil compatible Android 10+ (Android 10 à 15) :
    // Combine les données applicatives (data) et le conteneur android.notification avec priorité MAX
    // et catégorie call/high priority pour déclencher la Full-Screen Intent / bannière réveil d'écran
    const payload = {
        token: tokenDestinataire,
        data: {
            type: "APPEL",
    ...
    logCall("FCM_SENT", { callId, from, to, info: `[TEST 2 RÉVEIL VEILLE] Envoi push réveil d'écran (data + notification high priority Android 10+)` });
